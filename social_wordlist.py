...
Usage examples:
  python3 social_wordlist.py --name "Juan" --color "rojo" --birth-year 1995 \
    --dni 12345678 --min-length 8 --max-length 16 --pattern "7###C" \
    --output wordlist.txt

  python3 social_wordlist.py
...
DEFAULT_BASE_DIR = ""
DEFAULT_CONFIG_FILE = "social_wordlist.cfg"
DEFAULT_SPECIALS = "!@#$%^&*_+-="
DEFAULT_DIGITS = "0123456789"
DEFAULT_LOWER = "abcdefghijklmnopqrstuvwxyz"
...
def log_error(message):
    ...
    try:
        with open("social_wordlist_errors.log", "a", encoding="utf-8") as f:
...
def estimate_wordlist_size(config, social_tokens):
    ...
    # Factores de multiplicación aproximados
    multiplier = 1
    if config.get("lower"):
        multiplier *= 2  # mayúsculas/minúsculas
    if config.get("digits"):
        multiplier *= 1.5
    if config.get("specials"):
        multiplier *= 1.2
    if config.get("leet"):
        multiplier *= 1.3
    ...
    if patterns:
        pattern_multiplier = 0
        for pattern in patterns:
            hashes = pattern.count("#")
            if hashes:
                # Asumiendo pool de ~70 chars para #
                pattern_multiplier += 70 ** min(hashes, 3)  # Limitar explosión
        multiplier *= min(pattern_multiplier, 10000)  # Cap
    
    # Longitudes
    min_len = config.get("min_length", 4)
    max_len = config.get("max_length", 32)
    length_factor = max(1, (max_len - min_len + 1) / 10)
    multiplier *= length_factor
    
    # Complejidad
    complexity = config.get("complexity", 2)
    complexity_multiplier = {1: 0.5, 2: 1, 3: 2, 4: 3, 5: 5}
    multiplier *= complexity_multiplier.get(complexity, 1)
    
    estimated = int(base_count * multiplier)
    return min(estimated, 10000000)  # Cap a 10M para no asustar
...
def prompt_interactive(defaults=None):
    ...
    print("  • Para ayuda completa: python social_wordlist.py --help")
    ...
    print_question("02", "Color favorito de la persona objetivo")
    default_color = defaults.get("color")
    if default_color is not None:
        print(f"    ENTER = omitir | predeterminado: {get_default_label(format_default_value(default_color))}")
    else:
        print("    ENTER = omitir")
    user_input = input("    > ").strip()
    params["color"] = user_input if user_input else None
    print_question("03", "Fecha(s) de nacimiento de la persona objetivo (DD/MM/YYYY, DD-MM-YYYY o solo año)")
    ...
    print_question("04", "Otra(s) fecha(s) importante(s) de la persona objetivo (DD/MM/YYYY, DD-MM-YYYY o solo año)")
    ...
    print_question("07", "Equipo o deporte favorito de la persona objetivo")
    default_team = defaults.get("team")
    if default_team is not None:
        print(f"    ENTER = omitir | predeterminado: {get_default_label(format_default_value(default_team))}")
    else:
        print("    ENTER = omitir")
    user_input = input("    > ").strip()
    params["team"] = user_input if user_input else None
    print_question("08", "Ciudad o país de nacimiento de la persona objetivo")
    default_birth_place = defaults.get("birth_place")
    if default_birth_place is not None:
        print(f"    ENTER = omitir | predeterminado: {get_default_label(format_default_value(default_birth_place))}")
    else:
        print("    ENTER = omitir")
    user_input = input("    > ").strip()
    params["birth_place"] = user_input if user_input else None
    print_question("09", "Ciudad donde vive la persona objetivo")
    default_living_city = defaults.get("living_city")
    if default_living_city is not None:
        print(f"    ENTER = omitir | predeterminado: {get_default_label(format_default_value(default_living_city))}")
    else:
        print("    ENTER = omitir")
    user_input = input("    > ").strip()
    params["living_city"] = user_input if user_input else None
    print_question("10", "DNI o documento de la persona objetivo")
    ...
    print_question("11", "Descomponer fechas en fragmentos? (s/n) [n]")
    print("    ENTER = no")
    ...
    print_question("12", "Descomponer el documento de identidad en fragmentos? (s/n) [n]")
    print("    ENTER = no")
    ...
    print_question("13", "Mascota de la persona objetivo")
    default_pet = defaults.get("pet")
    if default_pet is not None:
        print(f"    ENTER = omitir | predeterminado: {get_default_label(format_default_value(default_pet))}")
    else:
        print("    ENTER = omitir")
    user_input = input("    > ").strip()
    params["pet"] = user_input if user_input else None
    print_question("14", "Cantante o banda favorita de la persona objetivo")
    default_singer = defaults.get("singer")
    if default_singer is not None:
        print(f"    ENTER = omitir | predeterminado: {get_default_label(format_default_value(default_singer))}")
    else:
        print("    ENTER = omitir")
    user_input = input("    > ").strip()
    params["singer"] = user_input if user_input else None
    others = []
    while True:
        print_question("15", "Otro dato genérico de la persona objetivo")
        print("    (empresa, hobby, apodo, nickname...)")
    ...
    params["dict_file"] = None  # No se pide en interactivo, se genera desde datos sociales
    ...
    while True:
        ...
        if os.path.isdir(base_dir):
            ...
            if retry != "r":
                params["base_dir"] = None
                break
    print_question("18", "Usar plantillas/patrones con #? (s/n) [n]")
    print("    ENTER = no")
    ...
    if mixed_case:
        ...
        if not params["lower"] and not params["upper"]:
            ...
            params["lower"] = True
            params["upper"] = True
    print_question("22", "Dígitos y especiales")
    print("    ENTER = sí a dígitos")
    ...
    print("    ENTER = sí a símbolos")
    params["specials"] = yes_no_input("    ¿Agregar variaciones con caracteres especiales? (s/n) [s]: ", default="s" if default_specials else "n")
    print_question("23", "Modo leet")
...
def apply_mutations(token, enable_leet=True, leet_mappings=None):
    ...
    if enable_leet:
        ...
        for idx, char in enumerate(variant):
            if char in leet_mappings:
                ...
                mutated_candidate = "".join(copy)
                if len(mutated_candidate) > 1 or any(c.isalnum() for c in mutated_candidate):
                    mutated.add(mutated_candidate)
    if token[::-1] != token:
        mutated.add(token[::-1])
    return mutated
...
def decompose_number(num_str):
    ...
    for i in range(len(num_str) - 2):
        fragments.add(num_str[i:i+3])
    # Subcadenas de longitud 2
    for i in range(len(num_str) - 1):
        fragments.add(num_str[i:i+2])
    # Con ceros a la izquierda si es corto
    if len(num_str) == 4:
        fragments.add(f"0{num_str[:3]}")
        fragments.add(f"{num_str[:3]}0")
    return fragments
...
def collect_social_tokens(params, dictionary_words, options):
    ...
    def process_value(value, should_decompose):
        ...
        for token in normalize_token(value, should_decompose):
            normalized.update(apply_mutations(token, options.get("leet", True), options.get("leet_mappings")))
            if token.isdigit():
    ...
    # No agregamos dígitos o símbolos aislados como tokens principales,
    # solo usamos los dígitos y símbolos válidos como variantes sobre datos sociales.
...
def generate_combination_variants(tokens, options, count_limit=None, max_length=16):
    ...
    if not tokens:
        return results
    max_combo = min(3, len(tokens))
    all_tokens = tokens
    for length in range(1, max_combo + 1):
        for subset in itertools.permutations(all_tokens, length):
            candidate = "".join(subset)
            if len(candidate) > max_length:
                continue
            if candidate not in results:
                results[candidate] = None
            if count_limit and len(results) >= count_limit:
                return results
    for token in list(results.keys()):
        if options.get("digits"):
            suffixes = ["123", "2023", "2024", "007"]
            birth_years = options.get("birth_year")
            if birth_years:
                if isinstance(birth_years, list):
                    suffixes.extend(birth_years)
                else:
                    suffixes.append(birth_years)
            for num in options.get("numeric_parts", []):
                if num:
                    suffixes.append(num)
            for suffix in suffixes:
                ...
                candidate2 = f"{suffix}{token}"
                if len(candidate2) <= max_length:
                    results.setdefault(candidate2, None)
        if options.get("specials"):
            for sym in DEFAULT_SPECIALS:
                candidate1 = f"{token}{sym}"
                if len(candidate1) <= max_length:
                    results.setdefault(candidate1, None)
                candidate2 = f"{sym}{token}"
                if len(candidate2) <= max_length:
                    results.setdefault(candidate2, None)
    if options.get("complexity", 2) >= 3:
        for token in list(results.keys()):
            if token:
                for variant in (token.capitalize(), token.lower(), token.upper(), token[::-1]):
                    if len(variant) <= max_length:
                        results.setdefault(variant, None)
    if options.get("complexity", 2) >= 4:
        for token in list(results.keys()):
            if token.isalpha():
                suffix1 = token + "!"
                if len(suffix1) <= max_length:
                    results.setdefault(suffix1, None)
                suffix2 = token + "2024"
                if len(suffix2) <= max_length:
                    results.setdefault(suffix2, None)
    if count_limit and len(results) > count_limit:
        return OrderedDict(list(results.items())[:count_limit])
    return results
    ...
    for pattern in patterns:
        ...
        if pattern.count("#") == 0:
            if min_length <= len(pattern) <= max_length:
                results.setdefault(pattern, None)
            continue
        if len(char_pool) == 0:
            continue
        if pattern.count("#") > 6:
            choices = char_pool[:10]
        else:
            choices = char_pool
        for product in itertools.product(choices, repeat=pattern.count("#")):
            candidate = []
            replacement_index = 0
            for char in pattern:
                if char == "#":
                    candidate.append(product[replacement_index])
                    replacement_index += 1
                else:
            ...
            if count_limit and len(results) >= count_limit:
                return results
            if len(results) > MAX_TEMPLATE_EXPANSION:
                break
    return results
...
def show_progress(stop_event, total_estimate, progress_state, output_file):
    ...
    line1 = f"Cargando: {spinner[0]}"
    line2 = f"{color_text('[', COLOR_YELLOW)}{color_text(base_name, COLOR_CYAN)}{color_text(']: ', COLOR_YELLOW)}{color_text('...', COLOR_YELLOW)}"
    ...
    while not stop_event.is_set():
        ...
        line1 = f"Cargando: {color_text(spinner_char, COLOR_GREEN)}"
        line2 = f"{color_text('[', COLOR_YELLOW)}{color_text(base_name, COLOR_CYAN)}{color_text(']: ', COLOR_YELLOW)}{color_text(current, COLOR_ORANGE)}"
        ...
        time.sleep(0.5)
    print()
    print("Generación completada o cancelada.")


def write_wordlist(file_path, candidates):
    try:
        with open(file_path, "w", encoding="utf-8") as f:
            for candidate in candidates:
                f.write(f"{candidate}\n")
        print(f"Wordlist guardada en: {file_path} ({len(candidates)} líneas)")
    except IOError as e:
...
def main():
    ...
    try:
        if config.get("patterns"):
            pattern_candidates = generate_from_patterns(config.get("patterns"), char_pool,
                                                        config["min_length"], config["max_length"], config.get("count"))
            for token in pattern_candidates:
                if config["min_length"] <= len(token) <= config["max_length"]:
                    if token not in candidates:
                        candidates[token] = None
                        print_live_candidate(token, progress_state)
        if config.get("count") is None or len(candidates) < config.get("count"):
            combo_candidates = generate_combination_variants(base_tokens, options, config.get("count"), config["max_length"])
            for token in combo_candidates:
                if len(candidates) >= (config.get("count") or float("inf")):
                    break
                if config["min_length"] <= len(token) <= config["max_length"]:
                    if token not in candidates:
                        candidates[token] = None
            ...
            for word in dictionary_words:
                if config["min_length"] <= len(word) <= config["max_length"]:
                    if word not in candidates:
                        ...
                        print_live_candidate(word, progress_state)
                    if config.get("count") and len(candidates) >= config.get("count"):
                        break

        final_candidates = list(candidates.keys())
        if config.get("count"):
            final_candidates = final_candidates[: config["count"]]
        ...
        if not final_candidates:
            ...
            write_wordlist(config.get("output_file", DEFAULT_OUTPUT_FILE), final_candidates)
    except KeyboardInterrupt:
        stop_event.set()
        progress_thread.join()
        print("\nGeneración cancelada por el usuario.")
        if candidates:
            final_candidates = list(filter(lambda x: config["min_length"] <= len(x) <= config["max_length"], candidates.keys()))
            if config.get("count"):
                final_candidates = final_candidates[: config["count"]]
            if final_candidates:
                write_wordlist(config.get("output_file", DEFAULT_OUTPUT_FILE), final_candidates)
                print("Se guardó lo generado hasta el momento.")
            else:
                print("No se generó nada antes de la cancelación.")
        else:
            print("No se generó nada antes de la cancelación.")
